...
//	               bu.setVisible(false);
	   ...
	   public void MyLocation() {  //물체랑 부딛히면 key를 비활성화 
	   ...
	   public void checkLocation() { // 물풍선 피격 판정 
	      // ǳ    ġ
...
//	      System.out.println();
	      ...
	      for (int i = 0; i < item.size(); i++) {
	         ...
	         // 496.525
	         y -= 40;
	         if ((bx + 40 >= item.get(i).getX() && bx + 40 <= item.get(i).getX() + 16)  //물풍선에 의한 블럭 파괴 판정 및 아이템 드랍 
...
//	         System.out.println();
	   ...
	   public void keyProcess() {
	      if (keyU == true) {
	         bazziCurrent("images/bazzi_back.png");  //캐릭터 움직임
	         myY -= 10;
	         bazzi.setLocation(myX, myY);
	         move = "U";
	   ...
	   public CookieMap(String username) {  //맵 셋팅 
	      ...
	      gt = new GameThread();   //이거 기점으로 자바파일 분리 
	      new Thread(runnable).start();
